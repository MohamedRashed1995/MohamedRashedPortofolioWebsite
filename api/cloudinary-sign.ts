...
async function verifyTokenWithBackend(authHeader: string): Promise<boolean> {
  try {
    const verifyUrl = `${getBackendOrigin()}/api/v1/admin/inquiries`;
    ...
    console.error(
      '[cloudinary-sign] Backend token verification failed:',
...
export default async function handler(
  ...
  const isValid = await verifyTokenWithBackend(authHeader);
