...
//   }
//   // Fallback to direct production backend URL if no env is set on Vercel
//   return PROD_BACKEND_URL;
// }

// export function buildApiUrl(endpoint: string): string {
//   if (!endpoint) return PROD_BACKEND_URL;
...
//   // Preserve Cloudinary signature endpoint if using serverless functions
//   if (path.startsWith('/api/cloudinary-sign')) {
//     return path;
//   }

//   // If running on production/Vercel, point directly to the backend
//   const base = getApiBaseUrl();
//   if (base) {
//     // Strip /api/proxy if present in path
...
//     schemaTables: dto.schemaTables || seedMatch?.databaseSchema,
...
//   const token = getAdminToken();
//   if (!token) {
//     throw new ApiError('Authentication token missing. Please log in as admin to upload images.', 401);
//   }
...
/**
 * Resolves the effective API base URL from Vite environment variables.
 * Disallows localhost URLs in client-side production builds to prevent SSR/CORS issues.
 */
export function getApiBaseUrl(): string {
  const configured = import.meta.env.VITE_API_BASE_URL;
  if (configured && typeof configured === 'string' && configured.trim().length > 0) {
...
/**
 * Normalizes relative routes into appropriate proxy or absolute API URLs.
 * Handles reverse-proxy routing (/api/proxy/...) for local and hosted environments.
 */
export function buildApiUrl(endpoint: string): string {
...
/**
 * Unified JSON/REST fetch client with automatic token clearance on 401 Unauthorized,
 * structured ApiError handling, and response content-type negotiation.
 */
export async function apiFetch<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const url = buildApiUrl(endpoint);
  const headers = new Headers(options.headers || {});
